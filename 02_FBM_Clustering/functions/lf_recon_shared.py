...
import functions.lf_recon_shared_config as C
...
def patient_freesurfer_dir(pid: str) -> Path:
    ...
    if pid.upper().startswith("PAT_"):
        # Shared layout: FreeSurfer subject folder is directly under ROOT_PAT/PAT_XXXX (contains mri/, surf/, label/).
        # Keep legacy fallback to avoid breaking older mirrors.
        shared = C.ROOT_PAT / pid
        if (shared / "mri").exists() and (shared / "surf").exists():
            return shared
...
# -------------------------
# Recon artifact generation (250_recon): contacts CSV + mosaic
# -------------------------
def _read_lines_text(path: Path) -> List[str]:
    return [ln.strip() for ln in path.read_text(encoding="utf-8", errors="ignore").splitlines() if ln.strip()]


def _parse_electrode_names(path: Path, *, skip_first_n: int = 0) -> List[str]:
    names = _read_lines_text(path)
    if skip_first_n > 0:
        names = names[int(skip_first_n):]
    return names


def _parse_pial_points(path: Path, *, skip_first_n: int = 0) -> np.ndarray:
    rows = []
    for ln in _read_lines_text(path):
    ...
    if skip_first_n > 0 and len(pts) >= skip_first_n:
        pts = pts[int(skip_first_n):]
    if pts.ndim != 2 or pts.shape[1] != 3:
        raise ValueError(f"PIAL file did not parse as Nx3 float coords: {path}")
    return pts


def _shaft_key_from_name(name: str) -> str:
    """Heuristic shaft grouping: take leading alpha prefix until first digit."""
    s = str(name).strip()
    m = re.match(r"^([A-Za-z]+)", s)
    return m.group(1).upper() if m else s.upper()


def _is_subdural_heuristic(name: str) -> bool:
    """Best-effort: mark as subdural if name suggests grid/strip."""
    s = str(name).lower()
    return ("grid" in s) or ("strip" in s) or s.startswith("g") or s.startswith("sg")


def _pv_mesh_from_fs_pial(subj_dir: Path, hemi: str) -> pv.PolyData:
    from nibabel.freesurfer.io import read_geometry

    path = subj_dir / "surf" / f"{hemi}.pial"
    if not path.exists():
...
def _compute_patient_cameras(bounds: Tuple[float, float, float, float, float, float]) -> Dict[str, Any]:
    """Re-use the same camera convention as fsaverage renders."""
    return compute_cameras(bounds)


def _render_contacts_view(
    lh_mesh: pv.PolyData,
    rh_mesh: pv.PolyData,
    names: List[str],
    points_tkr: np.ndarray,
    ...
    *,
    ...
    # cortex
    pl.add_mesh(lh_mesh, color=C.BRAIN_COLOR, opacity=0.25, specular=C.BRAIN_SPECULAR, specular_power=C.BRAIN_SPECULAR_POWER)
    pl.add_mesh(rh_mesh, color=C.BRAIN_COLOR, opacity=0.25, specular=C.BRAIN_SPECULAR, specular_power=C.BRAIN_SPECULAR_POWER)

    # shafts: group by alpha prefix
    by_shaft: Dict[str, List[int]] = {}
    for i, nm in enumerate(names):
        by_shaft.setdefault(_shaft_key_from_name(nm), []).append(i)

    # simple color cycle
    shaft_keys = sorted(by_shaft.keys())
    palette = list(mcolors.CSS4_COLORS.keys())
    for j, sk in enumerate(shaft_keys):
        idx = by_shaft[sk]
        pts = points_tkr[idx]
        col = palette[(j * 37) % len(palette)]
        # spheres per contact (depth vs subdural radius)
        r = np.where(is_subdural[idx], C.SUBDURAL_RADIUS, C.DEPTH_RADIUS)
        for p, rr in zip(pts, r):
            sph = pv.Sphere(radius=float(rr), center=(float(p[0]), float(p[1]), float(p[2])), theta_resolution=16, phi_resolution=16)
            pl.add_mesh(sph, color=col, opacity=C.ELECTRODE_OPACITY)

    pl.camera_position = cams[view]
    pl.reset_camera_clipping_range()
    img = pl.screenshot(transparent_background=transparent_bg, return_img=True, scale=C.SS_SCALE)
    pl.close()
    return img


def _stitch_horiz(imgs: List[np.ndarray]) -> np.ndarray:
    if not imgs:
        raise ValueError("No images to stitch")
    # Ensure same height
    h = min(im.shape[0] for im in imgs)
    imgs2 = [im[:h, :, :] for im in imgs]
    return np.concatenate(imgs2, axis=1)


def export_patient_contacts_tkrras_and_mosaic(
    pid: str,
    *,
    ...
    """Create <pid>_contacts_tkrRAS.csv and <pid>_mosaic.png under 250_recon.

    Inputs are strictly taken from the shared recon dataset (subjects_dir / pid):
      - mri/brainmask.mgz (header used for tkrRAS conventions)
      - surf/lh.pial + surf/rh.pial
      - elec_recon/<pid>.electrodeNames + elec_recon/<pid>.PIAL

    Output layout (fixed):
      <outputs_root>/<pid>/glassbrain/coords/<pid>_contacts_tkrRAS.csv
      <outputs_root>/<pid>/glassbrain/png/<pid>_mosaic.png
    """
    pid = str(pid).strip()
    subj_dir = (subjects_dir or C.SHARED_RECON_ROOT) / pid
    out_root = outputs_root or C.PAPER1_RECONS_ROOT
    transparent_bg = C.TRANSPARENT_BG if transparent_bg is None else bool(transparent_bg)

    # Required inputs
    t1_path = subj_dir / "mri" / "brainmask.mgz"
    if not t1_path.exists():
        raise FileNotFoundError(f"{pid}: missing {t1_path}")
    names_path = subj_dir / "elec_recon" / f"{pid}.electrodeNames"
    pial_path = subj_dir / "elec_recon" / f"{pid}.PIAL"
    if not names_path.exists():
        raise FileNotFoundError(f"{pid}: missing {names_path}")
    if not pial_path.exists():
        raise FileNotFoundError(f"{pid}: missing {pial_path}")

    # Outputs
    csv_dir = out_root / pid / C.BLOCK_NAME / C.CSV_PRODUCT
    png_dir = out_root / pid / C.BLOCK_NAME / "png"
    csv_dir.mkdir(parents=True, exist_ok=True)
    png_dir.mkdir(parents=True, exist_ok=True)

    csv_path = csv_dir / f"{pid}{C.CONTACTS_CSV_SUFFIX}"
    png_path = png_dir / f"{pid}_mosaic.png"
    if (not overwrite) and csv_path.exists() and png_path.exists():
        return {"csv_path": csv_path, "png_path": png_path}

    # Load surfaces
    lh = _pv_mesh_from_fs_pial(subj_dir, "lh")
    rh = _pv_mesh_from_fs_pial(subj_dir, "rh")

    # Load electrodes
    names = _parse_electrode_names(names_path, skip_first_n=names_skip_first_n)
    pts_tkr = _parse_pial_points(pial_path, skip_first_n=names_skip_first_n)
    if len(names) != len(pts_tkr):
        n = min(len(names), len(pts_tkr))
        names = names[:n]
        pts_tkr = pts_tkr[:n]

    # Hemisphere + dist-to-pial
    try:
        from scipy.spatial import cKDTree as KDTree
    except Exception as e:
        raise ImportError("scipy is required for KDTree distance computations.") from e

    lh_tree = KDTree(np.asarray(lh.points, dtype=np.float32))
    rh_tree = KDTree(np.asarray(rh.points, dtype=np.float32))
    d_lh, _ = lh_tree.query(pts_tkr, k=1)
    d_rh, _ = rh_tree.query(pts_tkr, k=1)
    is_left = (d_lh < d_rh).astype(np.int32)
    dist_to_pial = np.minimum(d_lh, d_rh).astype(np.float32)

    is_subd = np.asarray([_is_subdural_heuristic(nm) for nm in names], dtype=np.int32)
    ...
    df = pd.DataFrame({
        ...
        "isLeft": is_left,
        "isSubdural": is_subd,
        "dist_to_pial_mm": np.round(dist_to_pial, 2),
        "source_space": "tkrRAS",
    })
    df.to_csv(csv_path, index=False)

    # Render mosaic (left/frontal/right)
    bounds = (
    ...
    cams = _compute_patient_cameras(bounds)

    imgs = []
    for view in ["left", "frontal", "right"]:
        imgs.append(
            _render_contacts_view(
                lh, rh, names, pts_tkr, is_subd.astype(bool),
                cams, view,
                window_size=C.WINDOW_SIZE,
                transparent_bg=transparent_bg,
            )
        )
    mosaic = _stitch_horiz(imgs)
    try:
        import imageio.v3 as iio
        iio.imwrite(png_path, mosaic)
    except Exception:
        # fallback: save via PyVista textured plane
        pl = pv.Plotter(off_screen=True, window_size=(mosaic.shape[1], mosaic.shape[0]))
        tex = pv.numpy_to_texture(mosaic)
        plane = pv.Plane(i_size=mosaic.shape[1], j_size=mosaic.shape[0])
        pl.add_mesh(plane, texture=tex, show_edges=False)
        pl.show(auto_close=False)
        pl.screenshot(str(png_path), scale=1, transparent_background=transparent_bg)
        pl.close()

    return {"csv_path": csv_path, "png_path": png_path}


def batch_export_contacts_tkrras_and_mosaics(
    ...
    *,
    ...
    """Batch wrapper that returns a per-patient status table."""
    rows = []
    for pid in map(str, patient_ids):
        try:
            out = export_patient_contacts_tkrras_and_mosaic(
                pid,
                subjects_dir=subjects_dir,
                outputs_root=outputs_root,
                overwrite=overwrite,
                names_skip_first_n=names_skip_first_n,
            )
            rows.append({"patient_id": pid, "status": "OK", **{k: str(v) for k, v in out.items()}})
            print(f"[OK] {pid} -> {out['csv_path']} | {out['png_path']}")
        except Exception as e:
            rows.append({"patient_id": pid, "status": f"ERROR: {e}"})
            print(f"[ERROR] {pid}: {e}")
