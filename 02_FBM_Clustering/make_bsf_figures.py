...
def main() -> int:
    ...
    summ = json.loads((OUT / "part1_summary.json").read_text()) \
        if (OUT / "part1_summary.json").exists() else {}
    sep = pd.read_csv(OUT / "part1_separation.csv")
    sizes = pd.read_csv(OUT / "part1_cluster_sizes.csv")
    lopo = pd.read_csv(OUT / "part1_lopo.csv") if (OUT / "part1_lopo.csv").exists() else None
    labs = np.load(OUT / "part1_labels.npy")
    is_gated = np.load(OUT / "part1_is_gated.npy")
    xyz = np.load(OUT / "part1_xyz.npy")
    ...
    if f.exists():
        d = pd.read_csv(f)
        d = d[(d.feature_set == "concat_hg_all") & (d.scheme == "home")]
