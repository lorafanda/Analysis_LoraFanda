...
UI2_CSS = r"""
  ...
  #u2cens .u2cen { position:absolute; width:208px; padding:5px 6px 6px; cursor:pointer;
  ...
  #u2cens .u2cen canvas { display:block; width:208px; height:94px; border-radius:5px;
                          background:#ffffff; }
...
const U2_CW = 208, U2_CH = 94 + 21;          // card width, canvas + its one-line header
function u2Slots(n) {
...
function u2Cens() {
  ...
  const slots = u2Slots(items.length);
  box.innerHTML = items.map((x, q) => {
    const s = slots[q] || { x: 14, y: 14 };
    ...
    return `<div class="u2cen" style="left:${s.x}px;top:${s.y}px;--c:${u2CSS(u2Hue(x.i, K))}" `
         ...
         + `<canvas width="${U2_CW}" height="94"></canvas></div>`;
  }).join("");
  box.querySelectorAll(".u2cen").forEach((el, q) => {
    const cv = el.querySelector("canvas");
    try {
      renderCentroid(cv.getContext("2d"), cv.width, cv.height, Number(KSEL),
                     Number(items[q].d.cluster), CEN_REP, state.minLoad, false, cv.width / 380);
    } catch (e) { console.warn("overlay centroid:", e); }
    el.onclick = () => u2ClickChip(items[q].d.id, Number(items[q].d.cluster));
  });
}
