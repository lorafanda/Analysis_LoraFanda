...
def build_html(order):
    ...
    sub('  <div class="panel" id="readout">\n'
        ...
        '    <canvas id="cpCanvas" width="352" height="150"></canvas>\n'
        '    <div id="cpNote">—</div>\n'
    ...
    sub("  #readout b { color:#e8e8ec; }",
        ...
        "  #centroidPanel { bottom:16px; left:14px; width:380px; font-size:11.5px; line-height:1.45; }\n"
        "  #centroidPanel #cpTitle { font-size:12px; font-weight:600; color:var(--fg); margin:0 0 6px; }\n"
        "  #centroidPanel canvas { display:block; width:352px; height:150px; border-radius:6px; background:#0f1014; }\n"
...
let CEN = null;                 // {meta, X: Int16Array, W: Uint16Array|null} for RUN0
const _cenX = new Map();        // feature-set matrix, shared by every run on that set
const _cenLim = new Map();      // FIG 1's colour limits per run and K
async function loadCentroids(runId) {
  CEN = null;
  try {
    const meta = await (await fetch(CENTROIDS + runId + ".json", { cache: "no-store" })).json();
    let X = _cenX.get(meta.x_file);
    if (!X) { X = new Int16Array(await (await fetch(CENTROIDS + meta.x_file)).arrayBuffer()); _cenX.set(meta.x_file, X); }
    if (X.length !== meta.n * meta.ncond * meta.nband * meta.nt) throw new Error("matrix size does not match its json");
...
function cenStats(k, j, useMin, paper) {
  const m = CEN.meta, F = m.ncond * m.nband * m.nt, n = m.n, s = m.x_scale, X = CEN.X;
  const r = cenWeights(k, j, useMin, paper); if (!r) return null;
  const mean = new Float32Array(F), sd = new Float32Array(F);
  let sw = 0; for (let i = 0; i < n; i++) sw += r.w[i];
  if (sw <= 0) return Object.assign(r, { mean, sd, empty: true });
  for (let i = 0; i < n; i++) { const wi = r.w[i]; if (!wi) continue; const b = i * F; for (let f = 0; f < F; f++) mean[f] += wi * X[b + f]; }
  for (let f = 0; f < F; f++) mean[f] = mean[f] * s / sw;
  for (let i = 0; i < n; i++) { const wi = r.w[i]; if (!wi) continue; const b = i * F; for (let f = 0; f < F; f++) { const d = X[b + f] * s - mean[f]; sd[f] += wi * d * d; } }
  for (let f = 0; f < F; f++) sd[f] = Math.sqrt(sd[f] / sw);
  return Object.assign(r, { mean, sd, empty: false });
}
// FIG 1's scale: colour limit = 99th percentile of |argmax mean| over all clusters at
// this K; the line's y range spans every cluster's mean +/- SD. Cached per run and K.
function cenLimits(k) {
  const key = CEN.meta.id + ":" + k; if (_cenLim.has(key)) return _cenLim.get(key);
  const vals = []; let lo = 0, hi = 0;
  if (CEN.meta.labels[String(k)]) for (let j = 0; j < k; j++) {
    const st = cenStats(k, j, false, true); if (!st || st.empty) continue;
    for (let f = 0; f < st.mean.length; f++) { vals.push(Math.abs(st.mean[f])); lo = Math.min(lo, st.mean[f] - st.sd[f]); hi = Math.max(hi, st.mean[f] + st.sd[f]); }
  }
  vals.sort((a, b) => a - b);
...
function drawCentroid() {
  ...
  const d = curDef();
  if (!CEN || !d || d.cluster === undefined) { P.classList.add("hidden"); return; }
  ...
  const st = cenStats(k, j, true, paper);
  ...
  $("cpTitle").textContent = `#${pos || "?"} · Cluster ${j} · ` + (st.weighted
    ...
    : `n = ${st.nMembers} members`);
  $("cpNote").textContent = paper
    ? "paper definition: mean over argmax members, unweighted, ±1 SD — FIG 1's centroid"
    : CEN.W ? "loading-weighted mean over every electrode with loading ≥ the minimum, ±1 SD (weighted)"
  ...
  ctx.setLineDash([]); ctx.fillStyle = "#0f1014"; ctx.fillRect(0, 0, Wd, Hd);
  const lim = cenLimits(k), nc = m.ncond, nb = m.nband, nt = m.nt, ncol = nc * nt, col = clusterCSS(j);
  const padL = 6, padR = 6, padT = 6, padB = 16, w = Wd - padL - padR, h = Hd - padT - padB;
  if (st.empty) { ctx.fillStyle = "#9aa3ad"; ctx.font = "11px sans-serif"; ctx.textAlign = "left"; ctx.fillText("no electrode passes the minimum", padL + 8, Hd / 2); return; }
  if (nb > 1) {
    ...
    ctx.strokeStyle = "#111"; ctx.lineWidth = 1;                  // FIG 1's agreement outline
    const cw = w / ncol, ch = h / nb;
    for (let b = 0; b < nb; b++) for (let c = 0; c < nc; c++) for (let t = 0; t < nt; t++) {
      const f = (c * nb + b) * nt + t;
      if (st.sd[f] > 0 && Math.abs(st.mean[f]) > st.sd[f]) ctx.strokeRect(padL + (c * nt + t) * cw + 0.5, padT + (nb - 1 - b) * ch + 0.5, cw - 1, ch - 1);
    }
  } else {
    const lo = lim.ylo, hi = lim.yhi, sy = v => padT + h * (1 - (v - lo) / (hi - lo));
    ctx.strokeStyle = "#555"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(padL, sy(0)); ctx.lineTo(padL + w, sy(0)); ctx.stroke();
    for (let c = 0; c < nc; c++) {
  ...
  for (let c = 0; c < nc; c++) {                                  // blocks and the GO cue at 50%
    ...
    if (c) { ctx.strokeStyle = "#e8e8ec"; ctx.lineWidth = 1; ctx.beginPath(); ctx.moveTo(x0, padT); ctx.lineTo(x0, padT + h); ctx.stroke(); }
    ctx.strokeStyle = "#c9ced4"; ctx.setLineDash([4, 3]); ctx.beginPath(); ctx.moveTo(xm, padT); ctx.lineTo(xm, padT + h); ctx.stroke(); ctx.setLineDash([]);
    ctx.fillStyle = "#9aa3ad"; ctx.font = "10px sans-serif"; ctx.textAlign = "center"; ctx.fillText(m.conds[c], xm, Hd - 4);
  }
  ctx.textAlign = "right"; ctx.fillStyle = "#9aa3ad"; ctx.font = "9px sans-serif";
  ctx.fillText(nb > 1 ? `±${lim.vlim.toFixed(1)} dB` : `${lim.ylo.toFixed(1)}..${lim.yhi.toFixed(1)} dB`, Wd - padR, padT + 9);
}
// NOTHING RUNS HERE AT LOAD: this block sits above the line that defines $, and a call
// at module evaluation threw before boot() could start. The checkbox is wired in wireUI.
