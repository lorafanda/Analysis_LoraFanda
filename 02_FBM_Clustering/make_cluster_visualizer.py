...
// into it. cluster_patient_counts is per run AND per K, so the mark follows the K
// buttons and the matrix rather than being a property of a cluster id.
...
const u2DomOf = (cid, run) => u2Dom((((run || RUN) || {}).cluster_patient_counts || {})[String(cid)]);
const u2DomMark = d => (d && d.frac > 0.5) ? '<b class="dom" title="one patient holds more '
  + `than half of this cluster">*</b>` : "";
const u2DomText = d => (d && d.frac > 0.5)
  ? ` · ${d.pid} alone is ${Math.round(100 * d.frac)}% of it` : "";
...
function u2Chips() {
  ...
  box.innerHTML = defs.map((d, i) => {
    ...
    return `<button type="button" class="u2ch${on ? " on" : ""}" data-map="${u2esc(d.id)}" `
         ...
         + `n=${u2Size(d.cluster)}${u2esc(u2DomText(dom))}">`
...
async function u2Snap(p, box, w) {
  ...
  if (elecMesh) { try { nv.removeMesh(elecMesh); } catch (e) {} }
  elecMesh = null;
  return { im, cid, n: (D.sizes || {})[String(cid)], dom: u2Dom((D.pat || {})[String(cid)]) };
}
    ...
    for (let i = 0; i < panels.length; i++) {
      ...
      if (r && r.im && r.im.data) {
        ...
        fig.querySelector(".cap").innerHTML =
          ...
          + `<span class="lab">${u2esc(panels[i].sub)}`
          + `${r.n !== undefined ? " · n=" + r.n : ""}</span>`;
        fig.title = `cluster ${r.cid}${r.n !== undefined ? " · n=" + r.n : ""}`
                  + u2DomText(r.dom);
      } else {
