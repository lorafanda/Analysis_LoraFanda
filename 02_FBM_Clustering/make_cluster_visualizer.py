...
// representation of the same electrodes. No dispersion is drawn: the mean only.
const CENTROIDS = REPO + "02_FBM_Clustering/outputs/clustering/paper_web/centroids/";
const REP_LABEL = { concat_bands5z: "5 bands, z-scored", concat_hg: "HG (70-150 Hz)",
...
function cenMean(k, j, useMin, paper) {
  ...
  const mean = new Float32Array(g.F);
  let sw = 0; for (let i = 0; i < g.n; i++) sw += r.w[i];
  if (sw <= 0) return Object.assign(r, { mean, empty: true });
  ...
  return Object.assign(r, { mean, empty: false });
}
// FIG 1's colour limit: the 99th percentile of |argmax mean| over all clusters at this
// K, on the representation shown; the line's range spans every cluster's mean.
function cenLimits(k) {
  ...
  if (CEN.meta.labels[String(k)]) for (let j = 0; j < k; j++) {
    ...
    for (let f = 0; f < st.mean.length; f++) { const v = st.mean[f]; vals.push(Math.abs(v)); if (v < lo) lo = v; if (v > hi) hi = v; }
  }
  vals.sort((a, b) => a - b);
  const v = vals.length ? vals[Math.min(vals.length - 1, Math.floor(0.99 * vals.length))] : 1;
  const lim = { vlim: v || 1, ylo: lo * 1.1 - 0.05, yhi: hi * 1.1 + 0.05 }; _cenLim.set(key, lim); return lim;
}
...
function drawCentroid() {
  ...
  for (let c = 0; c < nc; c++) {                                  // blocks and the GO cue at 50%
    ...
    ctx.fillStyle = "#9aa3ad"; ctx.font = "10px sans-serif"; ctx.textAlign = "center"; ctx.fillText(g.conds[c], xm, Hd - 4);
  }
